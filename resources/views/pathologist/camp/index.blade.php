...
<section class="section show-section camp mt-3">
    <div class="addbutton">
        {{-- <a href="{{ route('pathologist.camp.create') }}" class="btn btn-primary">Add Camp</a> --}}
    </div>  
    <div class="row">
            ...
            <tbody>
                @foreach ($camps as $camp)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('pathologist.camp.edit', encrypt($camp->id)) }}"><i class="bi bi-pencil-square"></i></a>
    ...
    function deleteLabTechnician(campId)
    {
        ...
        .then((willDelClient) =>
        {
            if (willDelClient)
            {
                $.ajax({
                    type: "POST",
                    url: '{{ route("pathologist.camp.delete") }}',
                    headers: {
