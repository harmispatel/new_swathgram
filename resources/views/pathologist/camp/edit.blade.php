...
<section class="section store-form camps mt-3">
    <form action="{{ route('pathologist.camp.update') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="text-center mb-3 mt-5">
            <a href="{{ route('pathologist.camp') }}" type="button" class="btn btn-danger me-2">Cancel</a>
    ...
    $(document).ready(function () {
        $('#organization_type').change(function () {
            ...
            if (orgId) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('pathologist.organization.devices') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
