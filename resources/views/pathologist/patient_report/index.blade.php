...
<section class="section show-section patients mt-3">
    <div class="container">
        <form id="sort_blogs" action="{{ route('pathologist.patient.report') }}" method="GET">
            <div class="row">
                {{-- Organization --}}
...
<script>
    ...
    document.getElementById('clear-btn').addEventListener('click', function () {
        ...
        window.location.href = "{{ route('pathologist.patient.report') }}";
    });
    ...
    function deletePatient(patientId)
    {
        ...
        .then((willDelClient) =>
        {
            if (willDelClient)
            {
                $.ajax({
                    type: "POST",
                    url: '{{ route("pathologist.patient.delete") }}',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'id': patientId,
                    },
