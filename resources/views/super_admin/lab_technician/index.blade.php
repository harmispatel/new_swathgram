...
<section class="section show-section lab_technician mt-3">
    <div class="addbutton">
        <a href="{{ route('lab_technician.create') }}" class="btn btn-primary">Add Lab Technician</a>
    </div>  
    <div class="row">
            ...
            <tbody>
                @foreach ($lab_technicians as $lab_technician)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('lab_technician.edit', encrypt($lab_technician->id)) }}"><i class="bi bi-pencil-square"></i></a>
    ...
    function deleteLabTechnician(lab_technicianId)
    {
        ...
        .then((willDelClient) =>
        {
            if (willDelClient)
            {
                $.ajax({
                    type: "POST",
                    url: '{{ route("lab_technician.delete") }}',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'id': lab_technicianId,
                    },
