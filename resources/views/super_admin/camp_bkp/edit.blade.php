...
<section class="section store-form camps mt-3">
    <form action="{{ route('camp.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row mt-5">
            <div class="form-title d-flex">
                <i class="bi bi-person-fill"></i>
                <h5 class="ps-2 font-weight-bold">Signature</h5>
            </div>
        </div>
        <hr>
        ...
        <div class="row mt-5">
            <div class="col-md-6">
                <div class="row">
                    ...
                    <div class="col-md-9 d-flex">
                        <div class="form-check form-switch">
                            <label class="form-check-label ms-1" for="signature_1_status">ON/OFF</label>
                            <input name="signature_1_status" id="signature_1_status" class="form-check-input m-0" type="checkbox" role="switch" value="1" {{ $camp->signature_1_status ? 'checked' : '' }}>
                        </div>

                        <div class="signature-image ms-3">
                            <input type="hidden" name="signature_1" value="{{ $camp->signature_1 }}" id="signature_1" value="" class="form-control {{ $errors->has('signature_1') ? 'is-invalid' : '' }}">
                            <img id="signature_1_image" src="{{ $camp->signature_1 ? asset('public/super_admin_uploads/pathologist_signature_image/' . $camp->signature_1) : '' }}" alt="">
                            
                            @if($errors->has('signature_1'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('signature_1') }}
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="col-md-6">
                <div class="row">
                    ...
                    <div class="col-md-9 d-flex">
                        <div class="form-check form-switch">
                            <label class="form-check-label ms-1" for="signature_2_status">ON/OFF</label>
                            <input name="signature_2_status" class="form-check-input m-0" type="checkbox" role="switch" id="signature_2_status" value="1" {{ $camp->signature_2_status ? 'checked' : '' }}>
                        </div>

                        <div class="signature-image ms-3">
                            <input type="hidden" name="signature_2" value="{{ $camp->signature_2 }}" id="signature_2" class="form-control {{ $errors->has('signature_2') ? 'is-invalid' : '' }}">
                            <img id="signature_2_image" src="{{ $camp->signature_2 ? asset('public/super_admin_uploads/pathologist_signature_image/' . $camp->signature_2) : '' }}" alt="">
                            @if($errors->has('signature_2'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('signature_2') }}
                                </div>
                            @endif
                        </div>
                    </div>
        ...
        <div class="row mt-5">
            <div class="col-md-6">
                <div class="row">
                    <div class="col-md-3">
                        <label class="form-label">Signature 3</label>
                    </div>
                    <div class="col-md-9 d-flex">
                        <div class="form-check form-switch">
                            <label class="form-check-label ms-1" for="signature_3_status">ON/OFF</label>
                            <input name="signature_3_status" class="form-check-input m-0" type="checkbox" role="switch" id="signature_3_status" value="1" {{ $camp->signature_3_status ? 'checked' : '' }}>
                        </div>

                        <div class="signature-image ms-3">
                            <input type="hidden" name="signature_3" value="{{ $camp->signature_3 }}" class="form-control {{ $errors->has('signature_3') ? 'is-invalid' : '' }}">
                            <img id="signature_3_image" src="{{ $camp->signature_3 ? asset('public/super_admin_uploads/pathologist_signature_image/' . $camp->signature_3) : '' }}" alt="">

                            @if($errors->has('signature_3'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('signature_3') }}
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="col-md-6">
                <div class="row">
                    <div class="col-md-3">
                        <label class="form-label">Signature 4</label>
                    </div>
                    <div class="col-md-9 d-flex">
                        <div class="form-check form-switch">
                            <label class="form-check-label ms-1" for="signature_4_status">ON/OFF</label>
                            <input name="signature_4_status" id="signature_4_status" class="form-check-input m-0" type="checkbox" role="switch" id="signature_4_status" value="1" {{ $camp->signature_4_status ? 'checked' : '' }}>
                        </div>

                        <div class="signature-image ms-3">
                            <input type="hidden" name="signature_4" id="signature_4" value="{{ $camp->signature_4 }}" class="form-control {{ $errors->has('signature_4') ? 'is-invalid' : '' }}">
                            <img id="signature_4_image" src="{{ asset('public/super_admin_uploads/pathologist_signature_image/'.$camp->signature_4) }}" alt="" style="height: 100px;">

                            @if($errors->has('signature_4'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('signature_4') }}
                                </div>
                            @endif
                        </div>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        //pathologist image
        $('#pathologist').on('change', function () {
            let pathologistId = $(this).val();
            $('#signature-preview').hide();
            $('#signatureImages').empty();
            $('#selected_signature').val('');

            if (pathologistId) {
                $.ajax({
                    url: "{{ route('camp.signature') }}",
                    type: 'POST',
                    data: {
                        "_token": "{{ csrf_token() }}",
                    ...
                    success: function (response) {
                        if (response.success == 1 && response.signatures.length > 0) {
                           
                            let imageUrl = response.signatures[0];
                            let imageName = imageUrl.split('/').pop();

                            $('#signature_4_image').attr('src', imageUrl);
                            $('#signature_4').val(imageName);

                        } else {
                            $('#signature_4_image').attr('src', '');
                            $('#signature_4').val('');
                        }
                    },
                    error: function () {
