...
<section class="section show-section pathologist mt-3">
    <div class="addbutton">
        <a href="{{ route('admin.pathologist.create') }}" class="btn btn-primary">Add Pathologist</a>
    </div>  
    <div class="row">
            ...
            <tbody>
                @foreach ($pathologists as $pathologist)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.pathologist.edit', encrypt($pathologist->id)) }}"><i class="bi bi-pencil-square"></i></a>
    ...
    function deletePathologist(PathologistId)
    {
        ...
        .then((willDelClient) =>
        {
            if (willDelClient)
            {
                $.ajax({
                    type: "POST",
                    url: '{{ route("admin.pathologist.delete") }}',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'id': PathologistId,
                    },
