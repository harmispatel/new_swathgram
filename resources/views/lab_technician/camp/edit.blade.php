...
<section class="section store-form camps mt-3">
    <form action="{{ route('lab_technician.camp.update') }}" method="POST" enctype="multipart/form-data">
        @csrf
                         ...
                         <select name="lab_technician[]" id="lab_technician" class="form-control {{ $errors->has('lab_technician') ? 'is-invalid' : '' }}" multiple>
                            ...
                            @foreach ($lab_technicians as $lab_technician)
                                <option value="{{ $lab_technician->id }}" 
                                     {{ $camp->labTechnicians->contains('id', $lab_technician->id) ? 'selected' : '' }}>{{ $lab_technician->username }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="text-center mb-3 mt-5">
            <a href="{{ route('lab_technician.camp') }}" type="button" class="btn btn-danger me-2">Cancel</a>
    ...
    $(document).ready(function () {
        $('#organization_type').change(function () {
            ...
            if (orgId) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('lab_technician.organization.devices') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
