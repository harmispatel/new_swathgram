...
<section class="section show-section patients mt-3">
    <div class="addbutton">
        <a href="{{ route('lab_technician.patient.create') }}" class="btn btn-primary">Add Patients</a>
    </div>  
    <div class="container">
        <form id="sort_blogs" action="{{ route('lab_technician.patient.report') }}" method="GET">
            <div class="row">
                {{-- Organization --}}
...
<script>
    ...
    document.getElementById('clear-btn').addEventListener('click', function () {
        ...
        window.location.href = "{{ route('lab_technician.patient.report') }}";
    });
    ...
    function deletePatient(patientId)
    {
        ...
        .then((willDelClient) =>
        {
            if (willDelClient)
            {
                $.ajax({
                    type: "POST",
                    url: '{{ route("lab_technician.patient.delete") }}',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'id': patientId,
                    },
