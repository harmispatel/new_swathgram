...
class UserController extends Controller
{
    public function myProfile($id)
    {
        if(Auth::user()->role == 2)
        {
            ...
            return view('auth_admin.profile.super_admin_profile',$data);
        }
        // else
        // {
    ...
    public function editProfile($id)
    {
        ...
        {
            $data['user'] = User::where('id',decrypt($id))->first();
            return view('auth_admin.profile.super_admin_profile_edit',$data);
        }
        // else
        // {
        //     $data['user'] = User::with(['hasOneShop','hasOneSubscription'])->where('id',decrypt($id))->first();
    ...
    public function updateProfile(Request $request)
    {
        ...
        if(Auth::user()->role == 2)
        {
            ...
            $user->update();
            return redirect()->route('profile.view',encrypt($request->user_id))->with('success','Profile has been Updated SuccessFully..');
        }
    }
